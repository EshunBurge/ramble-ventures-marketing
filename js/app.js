...
const AI_AVAILABLE = typeof window.cowork !== "undefined" && typeof window.cowork.askClaude === "function";
...
async function regenerateIdeas(feedback){
  ...
  if(!AI_AVAILABLE){
    setRegenStatus("AI features require this dashboard to run inside Cowork.");
...
function toggleChat(){
  ...
  if(panel.classList.contains("open") && chatHistory.length === 0){
    if(AI_AVAILABLE){
      ...
      addChatMessage("system", "This chat requires the dashboard to run inside Cowork. Open it there to talk to the AI assistant.");
    }
  }
}
...
async function sendChat(){
  ...
  if(!AI_AVAILABLE){
    addChatMessage("ai", "AI chat is only available when this dashboard runs inside Cowork.");
    return;
  }
...
User message: ${text}`;
    ...
    let reply = typeof res === "string" ? res : (res?.text || res?.content?.[0]?.text || JSON.stringify(res));
    if(typeof reply !== "string") reply = "Here's what I found: " + JSON.stringify(reply);
    document.getElementById("rv-chat-messages").lastChild.textContent = reply;
  }catch(err){
